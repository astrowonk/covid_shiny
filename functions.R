...
get_state_data_covid_tracking <- function() {
    ...
    pop_data <- read_csv('SCPRC-EST2019-18+POP-RES.csv')
    state_json <- merge(state_json,pop_data,by.x='state',by.y='NAME',all.x=TRUE)
    state_json <- mutate(state_json,cases_per_100K = 100000 * (cases / POPESTIMATE2019), case_growth_per_100K = 100000 * (case_growth / POPESTIMATE2019))
