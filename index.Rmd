...
Virigina county-level data from the [Virginia Department of Health](http://www.vdh.virginia.gov/coronavirus/) is working again. 

You can select multiple states for the State Level view. Only the first state will be used for the county plots.
...
inputPanel(
  ...
  checkboxInput("plot_hospital","Plot Hospitalization Growth by County (Virginia Only)",value=FALSE),
...
renderUI({
  ...
  if (input$use_county_best & input$state[1] == 'Virginia') {
...
shiny::validate(need(input$county,message="Please wait.."))
   ...
   if (input$plot_hospital)
  {
    p <- plot_county_hospital(county_data, input$county, input$state[1], input$bins)
